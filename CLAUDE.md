...
```
cmd/server/main.go           # Server entry point (flags, accept loop, handleClient)
cmd/client/main.go           # Client entry point (flags, reconnect loop, connect)
...
internal/proto/              # Wire protocol: framed messages [1B type][4B len][payload], handshake, keepalive
